	...
	public String createCars(CarsDAO model, boolean isEdit){
		...
		try {
			...
			if(rs > 0 && isEdit != true) {
				...
				msg.showInfo("Success", "Record successfully created");
			}else
			{
				System.out.println("Success update");
				msg.showInfo("Success", "Record successfully updated");
			}
		...
		}catch(Exception err)
		{
			if(isEdit != true) {
				...
				msg.showError("Error", "createCars() error insert");
			}else
			{
				System.out.println("createCars() error update -->" + err.getMessage());
				msg.showError("Error", "createCars() error update");
			}
			err.printStackTrace();
		}finally {
			DataConnect.close(con);
